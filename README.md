...
🧩 [Composite Design Pattern](#-composite-design-pattern) : "Bütün-parça" ilişkisini yönetir ve nesneleri aynı yapıda işler.

💬 [Mediator Design Pattern](#-mediator-design-pattern) : Nesneler arasındaki iletişimi merkezi bir aracı üzerinden gerçekleştirir.
...
🖌️ [Decorator Design Pattern](#%EF%B8%8F-decorator-design-pattern) : Sınıflara dinamik olarak yeni işlevler ekler, mevcut sınıflara dokunmadan özellik katmayı mümkün kılar.
...
* Component (Bileşen): Tekil nesnelerin ve kompozit yapıların ortak özelliklerini tanımlar. Bu bileşen, Leaf ve Composite nesnelerin kullanacağı ortak bir arayüzdür. Örneğin, bir display veya add metodu burada tanımlanabilir.

* Leaf (Yaprak): Kompozit yapının en alt seviyesinde yer alan ve kendi başına işlem yapabilen bağımsız bileşenlerdir. Genellikle, alt bileşen içermezler ve en temel işlemleri sağlarlar.

* Composite (Kompozit): Diğer bileşenleri içerebilen bir bileşendir. Alt bileşenleri listeleyebilir, yeni bileşenler ekleyebilir veya mevcut bileşenleri çıkarabilir. Böylece, bir ağaç yapısındaki dalları temsil eder.
...
### ☀️ Mediator Tasarım Deseni'nin Amacı ve Avantajları
...
### ☀️ Mediator Tasarım Deseni’nin Bileşenleri

* Mediator (Aracı): Tüm nesneler arasındaki iletişimi yöneten merkezi bir bileşendir. Diğer nesnelerin taleplerini alır ve ilgili nesnelere iletir.

* Concrete Mediator (Somut Aracı): Mediator arayüzünü uygulayan, nesneler arasındaki iletişimi gerçekleştiren somut sınıftır. Talepleri alır, ilgili işlemleri yapar ve gerektiğinde diğer nesnelere iletir.

* Colleague (İş Arkadaşı): Mediator ile iletişime geçen nesnelerin arayüzüdür. Bu nesneler, birbirleriyle doğrudan iletişim kurmak yerine mediator üzerinden haberleşir.

* Concrete Colleague (Somut İş Arkadaşı): Colleague arayüzünü uygulayan nesnelerdir. Bu nesneler kendi işlevlerini gerçekleştirirken mediator aracılığıyla diğer nesnelerle iletişim kurabilirler.

### ☀️ Mediator Tasarım Desenini Kullanma Durumları
...
### ☀️ Mediator Design Pattern Çalışma Akışı
...
### ☀️ Mediator Design Pattern’in Avantajları
